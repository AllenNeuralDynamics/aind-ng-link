...
class ng_compressed_segmentation():
    ...
    def __init__(
            ...
            levels: List[int],
            chunk_size: int,
            compressed_encoding_size: int) -> None:
    ...
    @dask.delayed
    def compress_array(self, sub_array: np.array, factor: float, blocksize: int) -> list:
        """
                ...
                for z in range(data_dims[2]):
                    ...
                    output.append(
                        self.compress_array(
                            ...
                            level, self.compressed_encoding_size
                        )
                    )
    ...
    def block_to_bits(self, block: np.array, encoder_dict: dict, n_bits: int) -> np.array:
        """
