...
def generate_dispim_link(base_channel_xml_path: str, 
    ...
    channel_transforms = xml_parsing.extract_tile_transforms(cross_channel_xml_path)
    channel_transforms = {anchor_id: tfs[-1] for anchor_id, tfs in channel_transforms.items()}
    tmp = {}
    for channel_id, tfm in channel_transforms.items():
        nums = [float(val) for val in tfm['affine'].split(' ')]
        tmp[channel_id] = np.hstack((np.array([nums[0::4], nums[1::4], nums[2::4]]), 
                                     np.array(nums[3::4]).reshape(3, 1)))
    channel_transforms = tmp
    channel_transforms = {ch: ch_tf for ch, ch_tf in zip(channels, channel_transforms.values())}
    ...
    for channel, channel_tf in channel_transforms.items():
        ...
        for tile_id in range(len(intertile_transforms)): 
            ...
            net_matrix_3x3 = i_matrix_3x3 @ c_matrix_3x3  # NOTE: Right-multiply
